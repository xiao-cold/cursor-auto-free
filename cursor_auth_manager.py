...
class CursorAuthManager:
    ...
    def update_auth(self, email=None, access_token=None, refresh_token=None):
        ...
        if not updates:
            print(f"{Fore.YELLOW}{EMOJI['WARN']} 没有提供任何要更新的值{Style.RESET_ALL}")
            return False
        ...
        try:
            ...
            for key, value in updates:
                print(f"{Fore.YELLOW}{EMOJI['UPDATE']} 正在更新 {key.split('/')[-1]}...{Style.RESET_ALL}")
                query = "UPDATE itemTable SET value = ? WHERE key = ?"
                cursor.execute(query, (value, key))

                if cursor.rowcount > 0:
                    print(f"{Fore.GREEN}{EMOJI['SUCCESS']} {key.split('/')[-1]} 更新成功{Style.RESET_ALL}")
                else:
                    print(f"{Fore.YELLOW}{EMOJI['WARN']} {key.split('/')[-1]} 未找到或值未变化{Style.RESET_ALL}")
